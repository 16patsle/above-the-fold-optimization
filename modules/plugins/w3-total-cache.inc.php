...
class ABTFR_OPP_W3TotalCache extends ABTFR_OPP {
    /**
        ...
        if (!$this->config) {
            if (class_exists('\\W3TC\\Dispatcher')) {
                ...
                if ($this->config) {
                    ...
                    $minify_options = $this->config->get_array(
                        'minify.options'
                    );
                    $minify_options['postprocessor'] = array(
                        $this,
                        'process_minified_content'
                    );
    ...
    public function active($type = false) {
        if ($this->CTRL->plugins->active($this->plugin_file)) {
            ...
            switch ($type) {
                ...
                case 'js':
                    return $this->config->get_boolean('minify.enabled') &&
                        $this->config->get_boolean('minify.js.enable');
                    break;

                // CSS optimization?
                case 'css':
                    return $this->config->get_boolean('minify.enabled') &&
                        $this->config->get_boolean('minify.css.enable');
                    break;

                // HTML optimization?
                case 'html':
                    return $this->config->get_boolean('minify.enabled') &&
                        $this->config->get_boolean('minify.html.enable');
                    break;
            }
    ...
    public function skip_css($do_tag_minification, $style_tag, $file) {
        ...
        if (strpos($style_tag, 'rel="abtfr"') !== false) {
            return false;
        }
