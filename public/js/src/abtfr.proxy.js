...
Abtfr[CONFIG.LOAD_MODULE](function(window, Abtfr) {
     ...
     * Proxy setup
     */
    Abtfr[CONFIG.PROXY_SETUP] = function(cnf) {
        ...
        if (!PROXY_URL) {
            if (ABTFRDEBUG) {
                console.error('Abtfr.proxy()', 'no proxy url', cnf);
            }
        }
    ...
    var PROXIFY_URL = function(url, type) {
        ...
        if (PROXY_PRELOAD) {
            ...
            if (cachehash) {
                ...
                if (type === 'js') {
                    ...
                    if (typeof Abtfr[CONFIG.LOAD_CACHED_SCRIPT_URL] !== 'undefined') {
                        var parsedPath = PARSE_URL(path).href;
                        path = Abtfr[CONFIG.LOAD_CACHED_SCRIPT_URL](parsedPath);
                        if (ABTFRDEBUG) {
                            if (path !== parsedPath) {
                ...
                if (ABTFRDEBUG) {
                    if (localStorageUrl) {
                        if (regexmatch) {
                            console.log('Abtfr.proxy()', 'localStorage regex capture', Abtfr[CONFIG.LOCALURL](originalUrl), '➤', 'cache:' + cachehash, '➤', localStorageUrl);
                        } else {
                            console.log('Abtfr.proxy()', 'localStorage capture', Abtfr[CONFIG.LOCALURL](url), '➤', 'cache:' + cachehash, '➤', localStorageUrl);
                        }
                    } else {
                        if (regexmatch) {
                            console.log('Abtfr.proxy()', 'regex capture', Abtfr[CONFIG.LOCALURL](originalUrl), '➤', 'cache:' + cachehash);
                        } else {
                            console.log('Abtfr.proxy()', 'capture', Abtfr[CONFIG.LOCALURL](url), '➤', 'cache:' + cachehash);
                        }
                    }
        ...
        if (type === 'js') {
            ...
            if (typeof Abtfr[CONFIG.LOAD_CACHED_SCRIPT_URL] !== 'undefined') {
                ...
                url = Abtfr[CONFIG.LOAD_CACHED_SCRIPT_URL](parsedUrl);
                if (url !== parsedUrl) {

                    if (ABTFRDEBUG) {
                        if (regexmatch) {
                            console.log('Abtfr.proxy()', 'localStorage regex capture', Abtfr[CONFIG.LOCALURL](originalUrl), 'regex', '➤', Abtfr[CONFIG.LOCALURL](parsedUrl), '➤', url);
                        } else {
                            console.log('Abtfr.proxy()', 'localStorage capture', Abtfr[CONFIG.LOCALURL](parsedUrl), '➤', url);
                        }
                    }
        ...
        if (ABTFRDEBUG) {
            if (regexmatch) {
                console.log('Abtfr.proxy()', 'capture', Abtfr[CONFIG.LOCALURL](originalUrl), 'regex', '➤', url);
            } else {
                console.log('Abtfr.proxy()', 'capture', Abtfr[CONFIG.LOCALURL](url));
            }
        }
    ...
    var IS_IGNORED_SCRIPT = function(url) {
        ...
        if (PROXY_JS_INCLUDE) {
            ...
            if (!match) {
                ...
                if (ABTFRDEBUG) {
                    console.log('Abtfr.proxy()', 'ignore', Abtfr[CONFIG.LOCALURL](parser.href), 'not on include list');
                }
        ...
        if (PROXY_JS_EXCLUDE) {
            ...
            for (var i = 0; i < l; i++) {
                if (parser.href.indexOf(PROXY_JS_EXCLUDE[i]) !== -1) {
                    ...
                    if (ABTFRDEBUG) {
                        console.log('Abtfr.proxy()', 'ignore', Abtfr[CONFIG.LOCALURL](parser.href), 'on exclude list:', PROXY_JS_EXCLUDE[i]);
                    }
        ...
        if (PROXY_CSS_EXCLUDE) {
            ...
            for (var i = 0; i < l; i++) {
                if (parser.href.indexOf(PROXY_CSS_EXCLUDE[i]) !== -1) {
                    ...
                    if (ABTFRDEBUG) {
                        console.log('Abtfr.proxy()', 'ignore', Abtfr[CONFIG.LOCALURL](parser.href), 'on exclude list:', PROXY_CSS_EXCLUDE[i]);
                    }
    ...
    var IS_EXTERNAL_RESOURCE = function(node) {
        ...
        if (node.nodeName) {
            if (node.nodeName.toUpperCase() === 'SCRIPT') {
                ...
                if (node.hasAttribute('data-abtfr')) {
                    return false;
                }
                ...
                if (node.src) {
                    ...
                    if (!IS_EXTERNAL_SCRIPT(parser)) {
                        ...
                        if (typeof Abtfr[CONFIG.LOAD_CACHED_SCRIPT_URL] !== 'undefined') {
                            ...
                            url = Abtfr[CONFIG.LOAD_CACHED_SCRIPT_URL](parser.href);

                            if (url !== parser.href) {

                                if (ABTFRDEBUG) {
                                    console.log('Abtfr.proxy()', 'localStorage local capture', Abtfr[CONFIG.LOCALURL](parser.href), '➤', url);
                                }
                                ...
                                if (ABTFRDEBUG) {
                                    console.log('Abtfr.proxy()', 'localStorage local capture', Abtfr[CONFIG.LOCALURL](parser.href), '➤', 'bypass cache', '➤', url);
                                }
                            }
                        }
     ...
     * Proxify script
     */
    Abtfr[CONFIG.PROXIFY] = function(url) {
