...
Abtfr[CONFIG.LOAD_MODULE](function(window, Abtfr) {
    ...
    var LS = {
        ...
        getScript: function(url) {
            ...
            /** requestIdleCallback */
            if (Abtfr[CONFIG.IDLE] && typeof Abtfr[CONFIG.JS][2] !== 'undefined' && Abtfr[CONFIG.JS][2]) {

                var l = Abtfr[CONFIG.JS][2].length,
                    str;
                for (var i = 0; i < l; i++) {
                    if (typeof Abtfr[CONFIG.JS][2][i] !== 'object') {
                        continue;
                    }
                    if (url.indexOf(Abtfr[CONFIG.JS][2][i][0]) !== -1) {
                        idle = true;
                        if (Abtfr[CONFIG.JS][2][i][1]) {
                            idle_timeframe = Abtfr[CONFIG.JS][2][i][1];
                        }
                        break;
                    }
                }
        ...
        add: function(key, storeObj, retryCount) {
            ...
            if (typeof retryCount !== 'undefined' && parseInt(retryCount) > 10) {
                ...
                if (ABTFRDEBUG) {
                    console.error('Abtfr.js() ➤ localStorage quota reached', 'retry limit reached, abort saving...', key);
                }
                return;
            }
            ...
            try {
                ...
                if (e.name.toUpperCase().indexOf('QUOTA') >= 0) {
                    ...
                    if (tempScripts.length) {
                        ...
                        if (ABTFRDEBUG) {
                            console.error('Abtfr.js() ➤ localStorage quota reached', 'removed', tempScripts[0][0], 'for key', key);
                        }
                        ...
                        if (ABTFRDEBUG) {
                            console.error('Abtfr.js() ➤ localStorage quota reached', 'no files to remove');
                        }
                    ...
                    if (ABTFRDEBUG) {
                        console.error('Abtfr.js() ➤ localStorage error', e.name, e);
                    }
        ...
        clear: function(expired) {
            ...
            if (ABTFRDEBUG) {
                if (removed.length > 0) {
                    console.warn('Abtfr.js() ➤ localStorage cleared', removed.length, 'expired scripts');
                }
            }
        }
    ...
    var WEBWORKER = {
        ...
        stop: function() {
            if (this.worker) {
                ...
                if (ABTFRDEBUG) {
                    console.warn('Abtfr.js() ➤ web worker terminated');
                }
            }
        },
        ...
        handleMessage: function(event) {
            ...
            if (typeof WEBWORKER.scriptQueue[scriptIndex] === 'undefined') {
                ...
                // script not in queue
                if (ABTFRDEBUG) {
                    console.error('Abtfr.js() ➤ web worker script loader invalid response', response);
                }
                return;
            }
                ...
                if (ABTFRDEBUG) {
                    if (response[2] instanceof Array) {
                        if (parseInt(response[2][0]) > 200 && parseInt(response[2][0]) < 600) {
                            console.error('Abtfr.js() ➤ web worker ➤ ' + response[2][0] + ' ' + response[2][1], WEBWORKER.scriptQueue[scriptIndex].url);
                            return;
                        }
                    }
                    console.error('Abtfr.js() ➤ web worker script loader error', response[2]);
                }
                return;
            }
        },
        ...
        handleError: function(error) {
            ...
            if (ABTFRDEBUG) {
                console.error('Abtfr.js() ➤ web worker script loader error', error);
            }
        },
        ...
        if (OBJECT_URLS.length > 0) {
            ...
            for (var i = 0; i < l; i++) {
                try {
                    ...
                    if (ABTFRDEBUG) {
                        console.error('Abtfr.js() ➤ failed to revoke script url', OBJECT_URLS[i], err);
                    }
                }
            }
        }
    ...
    if (Abtfr[CONFIG.IDLE]) {
        ...
        Abtfr[CONFIG.ON_SCRIPT_LOAD](initClearTimeout);
    }

    /**
     * Load cached script
     */
    Abtfr[CONFIG.LOAD_CACHED_SCRIPT] = function(src, callback, onStart) {
        ...
        ASYNC(function() {
            ...
            if (url) {
                ...
                Abtfr[CONFIG.LOAD_SCRIPT](url, callback);
                return;
            }
             ...
             */
            Abtfr[CONFIG.LOAD_SCRIPT](src, function scriptLoaded() {
                ...
                WEBWORKER.loadScript(src, function onData(scriptData) {
                    ...
                    if (!scriptData) {
                        if (ABTFRDEBUG) {
                            console.error('Abtfr.js() ➤ web worker script loader no data', Abtfr[CONFIG.LOCALURL](src));
                        }
                        return;
                    }
                    ...
                    if (ABTFRDEBUG) {
                        if (scriptData instanceof Array) {
                            console.info('Abtfr.js() ➤ web worker ➤ localStorage saved chunked', '(' + scriptData.length + ' chunks)', Abtfr[CONFIG.LOCALURL](src));
                        } else {
                            console.info('Abtfr.js() ➤ web worker ➤ localStorage saved', '(' + scriptData.length + ')', Abtfr[CONFIG.LOCALURL](src));
                        }
                    }
     ...
     */
    Abtfr[CONFIG.PRELOAD_CACHED_SCRIPT] = function(url) {
        ASYNC(function() {
     ...
     */
    Abtfr[CONFIG.LOAD_CACHED_SCRIPT_URL] = function(src) {
