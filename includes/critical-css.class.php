...
class ABTFR_Critical_CSS {
    ...
    public function get_theme_criticalcss() {
        ...
        $criticalcss_files = is_dir($criticalcss_dir)
            ? array_diff(scandir($criticalcss_dir), array('..', '.'))
            : array();
        ...
        foreach ($criticalcss_files as $file) {
            ...
            if ($file === 'global.css') {
                if (
                    !isset($fileconfig[$file]) ||
                    filemtime($criticalcss_dir . $file) >
                        $fileconfig[$file]['t']
                ) {
                ...
                if (
                    ...
                    if ($css !== '') {
                        if (!preg_match('|^/\*(.*?)\*/|is', $css, $output)) {
                            $errors[] = array(
                                'message' =>
                                    'Critical CSS contains no config header.<br /><br /><textarea style="width:100%;height:150px;">' .
                                    htmlentities($css, ENT_COMPAT, 'utf-8') .
                                    '</textarea>'
                            );
                            continue;
                        }
                                            ...
                                            case 'filter':
                                                if (
                                                    ...
                                                    if (
                                                        ...
                                                        $errors[] = array(
                                                            'message' =>
                                                                'Failed to parse filter params for filter <strong>' .
                                                                htmlentities(
                                                                    trim(
                                                                        $filtername
                                                                    ),
                                ...
                                case 'prepend':
                                    ...
                                    if (
                                        ...
                                        if (substr($append, 0, 1) !== '/') {
                                            $appendPath =
                                                $criticalcss_dir . $append;
                                        }

                                        $appendPath = realpath($appendPath);
                                        if (
                                            strpos($appendPath, ABSPATH) ===
                                            false
                                        ) {
                                            ...
                                            wp_die(
                                                'Critical CSS file ' .
                                                    $file .
                                                    ' contains append/prepend location outside WordPress root.'
                                            );
                                        }
                ...
                if (
                    isset($fileconfig[$file]['append']) &&
                    !empty($fileconfig[$file]['append'])
                ) {
                    $fileconfig[$file]['append'] = array_unique(
                        $fileconfig[$file]['append']
                    );
                }
                if (
                    isset($fileconfig[$file]['prepend']) &&
                    !empty($fileconfig[$file]['prepend'])
                ) {
                    $fileconfig[$file]['prepend'] = array_unique(
                        $fileconfig[$file]['prepend']
                    );
                }
 ...
 *';
                ...
                case 'filter':
                    foreach ($condition_data as $filter_key => $filter_vars) {
                        ...
                        if (is_array($filter_vars) && !empty($filter_vars)) {
                            ...
                            $cssheader .=
                                ':' .
                                trim(json_encode($filter_vars, true), '[]');
                        }
                    }
                    break;
                default:
                    ...
                    if (
                        is_array($condition_data) &&
                        count($condition_data) > 0
                    ) {
                        $cssheader .=
                            ':' .
                            trim(json_encode($condition_data, true), '[]');
                    }
                    break;
            }
        }
        ...
        if (
            isset($config['matchType']) &&
            in_array($config['matchType'], array('any', 'all'))
        ) {
        ...
        if (isset($config['weight'])) {
            $cssheader .=
                "\n * @weight " .
                (is_numeric($config['weight']) && $config['weight'] >= 1
                    ? $config['weight']
                    : 1);
        }
    ...
    public function get() {
        ...
        if (!empty($criticalcss_files)) {
            ...
            foreach ($criticalcss_files as $file => $config) {
                ...
                if (
                    $primary_criticalcss &&
                    (!isset($config['appendToAny']) &&
                        !isset($config['prependToAny']))
                ) {
                    continue;
                }
                ...
                $matchType =
                    isset($config['matchType']) &&
                    strtolower($config['matchType']) === 'all'
                        ? 'all'
                        : 'any';
                        ...
                        if (isset($config[$type]) && !empty($config[$type])) {
                            foreach ($config[$type] as $append) {
                                ...
                                if (strpos($append, '/') !== false) {
                                    ...
                                    $appendcss = $this->get_file_contents(
                                        $appendPath
                                    );
                                } else {
                                    if (
                                        isset($criticalcss_files[$append]) &&
                                        file_exists(
                                            $criticalcss_files[$append]['file']
                                        )
                                    ) {
                                        $appendcss = $this->get_file_contents(
                                            $criticalcss_files[$append]['file']
        ...
        if (!$primary_criticalcss) {
            $cssdata = isset($criticalcss_files['global.css'])
        ...
        $servedfiles = '';
        if (
            isset($preAppendFiles['prepend']) &&
            !empty($preAppendFiles['prepend'])
        ) {
            $primary_criticalcss['css'] =
                implode(' ', array_values($preAppendFiles['prepend'])) .
                ' ' .
                $primary_criticalcss['css'];
        ...
        if (
            isset($preAppendFiles['append']) &&
            !empty($preAppendFiles['append'])
        ) {
            $primary_criticalcss['css'] =
                $primary_criticalcss['css'] .
                ' ' .
                implode(' ', array_values($preAppendFiles['append']));
         ...
         */
        if (
            $this->CTRL->view === 'no-critical-css' ||
            $this->CTRL->view === 'critical-css-creator-html'
        ) {
            if ($debug) {
                $criticalCSS .=
                    '
 ...
 *
' .
                    htmlentities($servedfiles, ENT_COMPAT, 'utf-8') .
                    $matchedconditions .
                    $debugnotice .
                    '
 */
...
';
    ...
    public function http2_cache_file_path($hash, $create = true) {
        ...
        if (!is_dir($cache_path)) {
            $this->error(
                'HTTP/2 CSS cache directory not available ' . $cache_path
            );
        }
     ...
     * Cache url
     */
    public function http2_cache_url(
        $hash,
