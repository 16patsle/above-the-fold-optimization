...
class ABTFR_Optimization {
    ...
    public function __construct(&$CTRL) {
        ...
        try {
            if (file_exists($config_index)) {
                $this->client_config_index = json_decode(
                    file_get_contents($config_index),
                    true
                );
            }
        } catch (Exception $err) {
            ...
            wp_die(
                'Failed to read ' .
                    str_replace(
                        ABSPATH,
                        '',
                        WPABTFR_PATH . 'public/js/src/config.index.json'
                    )
            );
        }
        ...
        foreach ($this->client_config_index as $position => $key) {
            if (is_string($key)) {
                ...
                foreach ($subkeys as $subposition => $subkey) {
                    $this->client_config_ref[$key . '-sub'][
                        $subkey
                    ] = $subposition;
                }
            }
        ...
        $this->optimize_css_delivery =
            isset($this->CTRL->options['cssdelivery']) &&
            intval($this->CTRL->options['cssdelivery']) === 1
                ? true
                : false;
         ...
         */
        $this->optimize_js_delivery =
            isset($this->CTRL->options['jsdelivery']) &&
            intval($this->CTRL->options['jsdelivery']) === 1
                ? true
                : false;
         ...
         */
        $this->html_replace =
            isset($this->CTRL->options['html_search_replace']) &&
            is_array($this->CTRL->options['html_search_replace']) &&
            !empty($this->CTRL->options['html_search_replace'])
                ? $this->CTRL->options['html_search_replace']
                : false;

        // ignore critical css view controller in javascript optimization
        if (
            $this->CTRL->view === 'critical-css-view' &&
            !defined('O10N_CSS_MODULE_LOADED')
        ) {
        ...
        if (
            ...
            $this->CTRL->view === 'critical-css-editor' &&
            !defined('O10N_CSS_MODULE_LOADED')
        ) {
            /**
             * Compare Critical CSS view
             */
            $this->CTRL->critical_css_test = new ABTFR_CriticalCSSEditor(
                $this->CTRL
            );
        } else {
            /**
    ...
    public function move_ob_to_front() {
        ...
        if (
            !empty($ob_callbacks) &&
            in_array(
                'ABTFR_Optimization::process_output_buffer',
                $ob_callbacks
            ) &&
        ...
        if (
            ...
            foreach ($wp_scripts->done as $handle) {
                if (
                    ...
                    $deps = array();
                    $handledeps =
                        isset($wp_scripts->registered[$handle]->deps) &&
                    ...
                    if (
                        !isset($wp_scripts->registered[$handle]->src) ||
                        trim($wp_scripts->registered[$handle]->src) === ''
                    ) {
                        // group reference
                        if (!empty($deps)) {
    ...
    public function process_output_buffer($buffer) {
        ...
        if (
            ...
            $ignorelist = array();
            if (
                isset($this->CTRL->options['cssdelivery_ignore']) &&
                !empty($this->CTRL->options['cssdelivery_ignore'])
            ) {
            ...
            $deletelist = array();
            if (
                isset($this->CTRL->options['cssdelivery_remove']) &&
                !empty($this->CTRL->options['cssdelivery_remove'])
            ) {
        ...
        if ($this->optimize_js_delivery && defined('O10N_JS_MODULE_LOADED')) {
            ...
            $ignorelist = array();
            if (
                isset($this->CTRL->options['jsdelivery_ignore']) &&
                !empty($this->CTRL->options['jsdelivery_ignore'])
            ) {
            ...
            $deletelist = array();
            if (
                isset($this->CTRL->options['jsdelivery_remove']) &&
                !empty($this->CTRL->options['jsdelivery_remove'])
            ) {
             ...
             */
            $forceAsync =
                isset($this->CTRL->options['jsdelivery_async_all']) &&
                intval($this->CTRL->options['jsdelivery_async_all']) === 1
                    ? true
                    : false;
            ...
            if (
                !$forceAsync &&
                isset($this->CTRL->options['jsdelivery_async']) &&
                !empty($this->CTRL->options['jsdelivery_async'])
            ) {
            ...
            if (
                isset($this->CTRL->options['jsdelivery_async_disabled']) &&
                !empty($this->CTRL->options['jsdelivery_async_disabled'])
            ) {
                foreach (
                    $this->CTRL->options['jsdelivery_async_disabled']
                    as $row
                ) {
             ...
             */
            if (
                isset($this->CTRL->options['jsdelivery_deps']) &&
                $this->CTRL->options['jsdelivery_deps']
            ) {
                list(
                    $wp_script_deps,
                    $wp_script_depgroups,
                    $wp_script_deprefs
                ) = $this->wp_script_dependencies();
            } else {
                ...
                foreach ($scripts as $script) {
                    ...
                    if (!empty($async_disabledlist)) {
                        foreach ($async_disabledlist as $disabled_file_string) {
                            if (
                                strpos($file, $disabled_file_string) !== false
                            ) {
                                $async = false;
                                break;
                            }
                    ...
                    if ($async === -1) {
                        $async =
                            strpos($matchedTag, ' async') !== false ||
                            strpos($matchedTag, ' defer') !== false;
                    }
        ...
        if ($this->optimize_js_delivery && defined('O10N_JS_MODULE_LOADED')) {
            ...
            if (defined('JSON_UNESCAPED_SLASHES')) {
                ...
                $scripts_json = str_replace(
                    '\\/',
                    '/',
                    json_encode($scripts_data)
                );
            }
        ...
        $searchreplace = apply_filters('abtfr_html_replace', array(
            $search,
            $replace,
            $search_regex,
            $replace_regex
        ...
        // strip comments
        if (
            isset($this->CTRL->options['html_comments']) &&
            $this->CTRL->options['html_comments']
        ) {
            // preserve comments
            $this->preserve_comments =
                isset($this->CTRL->options['html_comments_preserve']) &&
                !empty($this->CTRL->options['html_comments_preserve'])
                    ? $this->CTRL->options['html_comments_preserve']
                    : array();
        ...
        // minify HTML
        if (
            isset($this->CTRL->options['html_minify']) &&
            $this->CTRL->options['html_minify']
        ) {
    ...
    public function header() {
        ...
        if (
            ...
            require_once WPABTFR_PATH .
                'includes/critical-css-view-header.inc.php';
        }

        // debug enabled?
        $debug =
            current_user_can('administrator') &&
            intval($this->CTRL->options['debug']) === 1
                ? true
                : false;
        ...
        if (
            ...
            $this->http2push_criticalcss = $this->CTRL->criticalcss->http2_push_file(
                $inlineCSS
            );
        }
        ...
        if (
            ...
            $headCSS =
                $this->CTRL->options['cssdelivery_position'] === 'header'
                    ? true
                    : false;
        } else {
            // do not load CSS
        ...
        // print javascript
        print '<script ' .
            (!defined('ABTFR_NOREF') || !ABTFR_NOREF
                ? 'data-ref="https://goo.gl/C1gw96"'
                : '') .
            ' data-abtfr=\'' .
            str_replace('\'', '&#39;', json_encode($clientjs['config'])) .
            '\'>' .
            $clientjs['client'] .
            '</script>';
        ...
        // HTTP/2 Server Push Critical CSS
        if (
            $this->http2push_criticalcss &&
            !defined('O10N_CSS_MODULE_LOADED')
        ) {
            print '<link href="' .
                $this->http2push_criticalcss .
                '" media="all" rel="stylesheet" id="AbtfrCSS" data-abtfr/>';
        }
        ...
        if (
            ...
            print '<style type="text/css" id="AbtfrCSS" data-abtfr>' .
                $inlineCSS .
                '</style>';
        }
    }
    ...
    public function get_client_script($debug = false) {
        ...
        if ($this->CTRL->options['gwfo']) {
            ...
            $this->CTRL->gwfo->client_jssettings(
                $jssettings,
                $jsfiles,
                $inlineJS,
                $jsdebug,
         ...
         */
        $this->CTRL->pwa->client_jssettings(
            $jssettings,
            $jsfiles,
            $script_code,
            $jsdebug,
        ...
        if (
            ...
            // get proxy client
            $this->CTRL->proxy->client_jssettings(
                $jssettings,
                $jsfiles,
                $jsdebug
            );
        ...
        if ($this->optimize_js_delivery && defined('O10N_JS_MODULE_LOADED')) {
            // jQuery ready stub
            if (
                isset($this->CTRL->options['jsdelivery_jquery']) &&
                $this->CTRL->options['jsdelivery_jquery']
            ) {
                $jsfiles[] =
                    WPABTFR_PATH .
                    'public/js/abtfr-jquery-stub' .
                    $jsdebug .
                    '.min.js';
            }

            $jsfiles[] =
                WPABTFR_PATH . 'public/js/abtfr-js' . $jsdebug . '.min.js';

            // script loader
            if (
                isset($this->CTRL->options['jsdelivery_scriptloader']) &&
                $this->CTRL->options['jsdelivery_scriptloader'] !==
                    'little-loader'
            ) {
                ...
                if (
                    ...
                    $jsfiles[] =
                        WPABTFR_PATH .
                        'public/js/abtfr-js-localstorage' .
                        $jsdebug .
                        '.min.js';
                }
            }
        ...
        if (
            ...
            $jsfiles[] =
                WPABTFR_PATH . 'public/js/abtfr-css' . $jsdebug . '.min.js';

            /** Async CSS controller */
            if (intval($this->CTRL->options['loadcss_enhanced']) === 1) {
                $jsfiles[] =
                    WPABTFR_PATH .
                    'public/js/abtfr-loadcss-enhanced' .
                    $jsdebug .
                    '.min.js';
            } else {
                $jsfiles[] =
                    WPABTFR_PATH .
                    'public/js/abtfr-loadcss' .
                    $jsdebug .
                    '.min.js';
            }
        }
         ...
         */
        if ($this->optimize_js_delivery && defined('O10N_JS_MODULE_LOADED')) {
            $jssettings[$this->client_config_ref['js']] = array(
                $this->js_replacement_string,
                $this->CTRL->options['jsdelivery_position'] === 'footer'
                    ? true
                    : false
            );
            if (
                isset($this->CTRL->options['jsdelivery_idle']) &&
                !empty($this->CTRL->options['jsdelivery_idle'])
            ) {
                $jssettings[$this->client_config_ref['js']][] =
                    $this->CTRL->options['jsdelivery_idle'];
            }
        }
        ...
        if (
            ...
            $jssettings[$this->client_config_ref['css']] =
                $this->criticalcss_replacement_string;

            if (
                isset($this->CTRL->options['cssdelivery_renderdelay']) &&
                intval($this->CTRL->options['cssdelivery_renderdelay']) > 0
            ) {
                $jssettings[$this->client_config_ref['css_delay']] = intval(
                    $this->CTRL->options['cssdelivery_renderdelay']
                );
            }

            if (
                !isset($this->CTRL->options['cssdelivery_position']) ||
                $this->CTRL->options['cssdelivery_position'] !== 'header'
            ) {
        ...
        return array(
            ...
            'client' => trim(
                $script_code_before .
                    ...
                    $script_code .
                    'Abtfr[' .
                    $this->client_config_ref['header_load'] .
                    ']();'
            )
        );
    }
     ...
     */
    public function get_client_script_hash(
        $debug = false,
        $algorithm = 'sha256'
    ) {
