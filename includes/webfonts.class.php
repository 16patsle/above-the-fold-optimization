...
class ABTFR_WebFonts {
    ...
    public function __construct(&$CTRL) {
        ...
        if (
            isset($this->CTRL->options['gwfo_googlefonts']) &&
            is_array($this->CTRL->options['gwfo_googlefonts'])
        ) {
            $this->googlefonts = $this->CTRL->options['gwfo_googlefonts'];
        }
        if (
            isset($this->CTRL->options['gwfo_googlefonts_ignore']) &&
            is_array($this->CTRL->options['gwfo_googlefonts_ignore'])
        ) {
            $this->googlefonts_ignore =
                $this->CTRL->options['gwfo_googlefonts_ignore'];
        }

        // autodetect google fonts
        $this->googlefonts_autodetect =
            !isset($this->CTRL->options['gwfo_googlefonts_auto']) ||
            $this->CTRL->options['gwfo_googlefonts_auto']
                ? true
                : false;
        ...
        if ($this->CTRL->options['gwfo']) {
            ...
            if (
                isset($this->CTRL->options['gwfo_loadmethod']) &&
                $this->CTRL->options['gwfo_loadmethod'] === 'wordpress'
            ) {
                /**
        ...
        if (
            ...
            foreach ($out[0] as $n => $fontLink) {
                if (substr_count($fontLink, 'fonts.googleapis.com/css') > 0) {
                    ...
                    // parse fonts
                    list($fonts, $ignoredFontsLink) = $this->fonts_from_url(
                        $fontLink
                    );
                    ...
                    if ($fonts && !empty($fonts['google'])) {
                        foreach ($fonts['google'] as $googlefont) {
                            if (
                                !in_array(trim($googlefont), $this->googlefonts)
                            ) {
                        ...
                        $replace[] = $ignoredFontsLink
                            ? '@import ' . $ignoredFontsLink . ';'
                            : ' ';
                    }
                }
            }
    ...
    public function process_cssfile($cssfile) {
        ...
        if (strpos($cssfile, 'fonts.googleapis.com/css') !== false) {
            ...
            if ($fonts && !empty($fonts['google'])) {
                ...
                return $ignoredFontsLink
                    ? array($ignoredFontsLink, 'ignore')
                    : 'delete';
            }
        }
    ...
    public function process_html_pre($HTML) {
        ...
        if (strpos($HTML, 'WebFontConfig') !== false) {
            ...
            if (
                preg_match_all('#WebFontConfig\s*=\s*\{[^;]+\};#s', $HTML, $out)
            ) {
    ...
    public function replace_html($searchreplace) {
        ...
        if (isset($this->CTRL->options['gwfo_loadmethod'])) {
            if ($this->CTRL->options['gwfo_loadmethod'] !== 'disabled') {
                ...
                $search[] =
                    '"' . $this->webfont_inline_replacement_string . '"';
        ...
        if (isset($urlParameters['text'])) {
            ...
            foreach ($familyParams as $familyKey => $fontFamilies) {
                ...
                if (strlen($fontFamily[0]) > 0) {
                    // font reference
                    $fontstr =
                        $fontFamily[0] . ':' . $fontFamily[1] . ':' . $subset;
            ...
            if (
                ...
                if (is_array($gfonts) && !empty($gfonts)) {
                    $googlefonts = array_unique(
                        array_merge($googlefonts, $gfonts)
                    );
                }
            }
        }
    ...
    public function get_google_fonts() {
         ...
         */
        if (
            !empty($this->googlefonts) &&
            isset($this->CTRL->options['gwfo_googlefonts_remove']) &&
            !empty($this->CTRL->options['gwfo_googlefonts_remove'])
        ) {
            $removeList = $this->CTRL->options['gwfo_googlefonts_remove'];
            $this->googlefonts = array_filter($this->googlefonts, function (
                $font
            ) use ($removeList) {
...
REGEX;
    ...
    public function enqueue_webfontjs() {
         ...
         */
        $in_footer =
            isset($this->CTRL->options['gwfo_loadposition']) &&
            $this->CTRL->options['gwfo_loadposition'] === 'footer'
                ? true
                : false;
        wp_enqueue_script(
            'abtfr_webfontjs',
            WPABTFR_URI . 'public/js/webfont.js',
            array(),
    ...
    public function package_version($reset = false) {
        ...
        $webfont_package =
            WPABTFR_PATH . 'public/js/src/webfontjs_package.json';
        if (!file_exists($webfont_package)) {
            $this->CTRL->admin->set_notice(
                'PLUGIN INSTALLATION NOT COMPLETE, MISSING public/js/src/webfontjs_package.json',
                'ERROR'
            );
            ...
            if (!is_array($package)) {
                $this->CTRL->admin->set_notice(
                    'failed to parse public/js/src/webfontjs_package.json',
                    'ERROR'
                );
    ...
    public function client_jssettings(
        ...
        if (
            isset($this->CTRL->options['gwfo_loadmethod']) &&
            $this->CTRL->options['gwfo_loadmethod'] === 'disabled'
        ) {
             ...
             */
            if (
                isset($this->CTRL->options['gwfo_config']) &&
                $this->CTRL->options['gwfo_config'] !== '' &&
                isset($this->CTRL->options['gwfo_config_valid']) &&
                $this->CTRL->options['gwfo_config_valid']
            ) {
                $WebFontConfig = trim($this->CTRL->options['gwfo_config']);

                // WebFontConfig has Google fonts, merge
                if (
                    strpos($WebFontConfig, 'GOOGLE-FONTS-FROM-INCLUDE-LIST') !==
                    false
                ) {
                    $quote =
                        strpos(
                            ...
                            ? '\''
                            : '"';
                    $WebFontConfig = str_replace(
                        $quote . 'GOOGLE-FONTS-FROM-INCLUDE-LIST' . $quote,
                        json_encode($this->googlefonts),
                        $WebFontConfig
                    );
                }
            ...
            if (empty($google_fonts)) {
                ...
                $jssettings[$gfwindex][
                    $this->CTRL->optimization->client_config_ref['gwf-sub'][
                        'google_fonts'
                    ]
                ] = $this->webfont_inline_replacement_string;
                ...
                // async
                if (
                    $this->CTRL->options['gwfo_loadmethod'] === 'async' ||
                    $this->CTRL->options['gwfo_loadmethod'] === 'async_cdn'
                ) {
                    $jssettings[$gfwindex][
                        $this->CTRL->optimization->client_config_ref['gwf-sub'][
                            'async'
