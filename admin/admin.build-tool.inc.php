...
$taskname = isset($_REQUEST['taskname'])
    ? trim($_REQUEST['taskname'])
    : (isset($default['taskname'])
        ? trim($default['taskname'])
        : '');
$dimensions = isset($_REQUEST['dimensions'])
    ? $_REQUEST['dimensions']
...
<nav class="subnav">
	...
	<a href="<?php echo add_query_arg(
     ...
     admin_url('admin.php')
 ); ?>" class="f">Quality Test (mirror)</a><a href="<?php echo add_query_arg(
    array('page' => 'abtfr-build-tool'),
    admin_url('admin.php')
); ?>" class="s">Gulp.js Critical CSS Generator</a>
</nav>
<form method="post" action="<?php echo admin_url(
    'admin-post.php?action=abtfr_create_critical_package'
); ?>" class="clearfix abtfr-bt-builder">
												...
												<table class="form-table">
													<tr valign="top">
														...
														<td style="padding-bottom:0px;">
						                                    <input type="text" name="taskname" size="50" value="<?php if (
                                              $taskname
                                          ) {
                                              print htmlentities(
                                                  $taskname,
                                                  ...
                                                  'utf-8'
													...
													<tr valign="top">
														...
														<td style="padding-top:0px;;padding-bottom:0px;">
						                                    <textarea style="width: 100%;height:50px;font-size:11px;" name="dimensions" placeholder="Leave blank for the default dimension..."><?php if (
                                              $dimensions
                                          ) {
                                              print $this->CTRL->admin->newline_array_string(
                                                  $dimensions
                                              );
                                          } ?></textarea>
													...
													<tr valign="top">
														...
														<td>
						                                    <label><input type="checkbox" name="extra" value="1"<?php if (
                                              $extra
                                          ) {
                                              print ' checked';
                                          } ?>> Enabled</label>
                                                ...
                                                print ' selected="selected"';
						                                     	...
						                                     	<option value="global.css"<?php if (
                                                $update === 'global'
                                            ) {
                                                print ' selected="selected"';
...
if ($criticalcss_files && count($criticalcss_files) > 1) {
    ...
    foreach ($criticalcss_files as $file => $config) {
        ...
        print '<option value="' .
            ...
            '"' .
            ($update === $file ? ' selected="selected"' : '') .
            '>Overwrite ' .
            htmlentities($file . ' - ' . $config['name'], ENT_COMPAT, 'utf-8') .
            '</option>';
    }
...
?>
							...
							<p>Test the quality of the created critical path CSS using the <a href="<?php print add_query_arg(
           array('page' => 'abtfr-compare'),
           admin_url('admin.php')
       ); ?>">Critical CSS Quality Test</a> and optionally use the file <code>extra.css</code> to fix problems in the generated Critical Path CSS.</p>
