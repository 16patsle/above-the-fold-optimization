...
class ABTFR_Admin_Proxy {
    ...
    public function update_settings() {
         ...
         * Proxy settings
         */
        $options['proxy_url'] = isset($input['proxy_url'])
            ? trim($input['proxy_url'])
            : '';
        if ($options['proxy_url']) {
            if (!preg_match('|^http(s)?://|Ui', $options['proxy_url'])) {
                $this->CTRL->admin->set_notice(
                    '<p style="font-size:18px;">Invalid proxy url.</p>',
                    'ERROR'
                );
            ...
            if (strpos($options['proxy_url'], '{PROXY:URL}') === false) {
                $this->CTRL->admin->set_notice(
                    '<p style="font-size:18px;">Proxy url does not contain <code>{PROXY:URL}</code>.</p>',
                    'ERROR'
                );
        ...
        // CDN
        $options['proxy_cdn'] = isset($input['proxy_cdn'])
            ? trim($input['proxy_cdn'])
            : '';
        if ($options['proxy_cdn'] !== '') {
            if (
                !preg_match('|^http(s)?://[a-z0-9]|Ui', $options['proxy_cdn'])
            ) {
                $this->CTRL->admin->set_notice(
                    '<p style="font-size:18px;">Proxy CDN url is not valid (only http:// and https:// urls are allowed).</p>',
                    'ERROR'
                );
                $options['proxy_cdn'] = '';
            }
        ...
        // CSS proxy
        $options['css_proxy'] =
            isset($input['css_proxy']) && intval($input['css_proxy']) === 1
                ? true
                : false;
        $options['css_proxy_include'] = $this->CTRL->admin->newline_array(
            $input['css_proxy_include']
        );
        $options['css_proxy_exclude'] = $this->CTRL->admin->newline_array(
            $input['css_proxy_exclude']
        );

        // Javascript proxy
        $options['js_proxy'] =
            isset($input['js_proxy']) && intval($input['js_proxy']) === 1
                ? true
                : false;
        $options['js_proxy_include'] = $this->CTRL->admin->newline_array(
            $input['js_proxy_include']
        );
        $options['js_proxy_exclude'] = $this->CTRL->admin->newline_array(
            $input['js_proxy_exclude']
        );

        // preload urls
        $preload_urls = array(
            'css' => $this->CTRL->admin->newline_array(
                $input['css_proxy_preload']
            ),
            'js' => $this->CTRL->admin->newline_array(
                $input['js_proxy_preload']
            )
        );
                        ...
                        if (!is_array($url_config)) {
                            $this->CTRL->admin->set_notice(
                                'The ' .
                                    ...
                                    '</code> was not recognized as valid JSON.',
                                'ERROR'
                            );
                            continue;
                        }
                        if (!isset($url_config['url'])) {
                            $this->CTRL->admin->set_notice(
                                'The ' .
                                    ...
                                    ' preload JSON <code>' .
                                    htmlentities($url, ENT_COMPAT, 'utf-8') .
                                    '</code> does not contain a target url.',
                                'ERROR'
                            );
                            // no target url
                            continue;
                                ...
                                if (
                                    ...
                                    $this->CTRL->admin->set_notice(
                                        'The ' .
                                            ...
                                            '</code> contains an invalid expire time.',
                                        'ERROR'
                                    );
                                ...
                                if ($valid === false || $error) {
                                    $this->CTRL->admin->set_notice(
                                        'The ' .
                                            ...
                                            '</code> contains an invalid regular expression.' .
                                            ($error
                                                ? '<br /><p>Error: ' .
                                                    $error .
                                                    '</p>'
                                ...
                                if (
                                    ...
                                    $this->CTRL->admin->set_notice(
                                        'The ' .
                                            ...
                                            '</code> does not contain a valid CDN url.',
                                        'ERROR'
                                    );
                                    // no target url
                                    continue;
