...
class ABTFR_Admin_CSS {
    ...
    public function update_settings() {
        ...
        if (isset($_POST['uploadgooglefontzip'])) {
            ...
            if (
                !empty($_FILES) &&
                $_FILES['googlefontzip'] &&
                $_FILES['googlefontzip']['name']
            ) {
                if ($_FILES['googlefontzip']['error']) {
                    $this->CTRL->admin->set_notice(
                        'Google Font zip-file upload error: ' .
                            $_FILES['googlefontzip']['error'],
                        'ERROR'
                    );
                } else {
                    if (!class_exists('ZipArchive')) {
                        $this->CTRL->admin->set_notice(
                            'Your PHP installation does not support <a href="http://php.net/manual/en/ziparchive.extractto.php" target="_blank">ZipArchive</a>.',
                            'ERROR'
                        );
                    } else {
                        $fontsdir =
                        ...
                        $zip = new ZipArchive();
                        if (
                            $zip->open($_FILES['googlefontzip']['tmp_name']) ===
                            true
                        ) {
                            ...
                            foreach ($fontfiles as $fontfile) {
                                // font files only
                                if (
                                    is_file($fontsdir . $fontfile) &&
                                    preg_match(
                                        '#\.(eot|woff|woff2|svg|ttf)$#Ui',
                                        $fontfile
                                    )
                                ) {
                            ...
                            $this->CTRL->admin->set_notice(
                                '<p style="font-size:18px;">Google Font zip-file extracted successfully.</p>',
                                'NOTICE'
                            );
                        } else {
                            $this->CTRL->admin->set_notice(
                                'Failed to read Google Font zip-file.',
                                'ERROR'
                            );
                        }
                    }
         ...
         */
        $options['cssdelivery'] =
            isset($input['cssdelivery']) && intval($input['cssdelivery']) === 1
                ? true
                : false;
        $options['loadcss_enhanced'] =
            isset($input['loadcss_enhanced']) &&
            intval($input['loadcss_enhanced']) === 1
                ? true
                : false;
        $options['cssdelivery_position'] = trim($input['cssdelivery_position']);
        $options['cssdelivery_ignore'] = $this->CTRL->admin->newline_array(
            $input['cssdelivery_ignore']
        );
        $options['cssdelivery_remove'] = $this->CTRL->admin->newline_array(
            $input['cssdelivery_remove']
        );
        $options['cssdelivery_renderdelay'] =
            isset($input['cssdelivery_renderdelay']) &&
            is_numeric($input['cssdelivery_renderdelay']) &&
            intval($input['cssdelivery_renderdelay']) > 0
                ? intval($input['cssdelivery_renderdelay'])
                : false;
         ...
         */
        $options['gwfo'] =
            isset($input['gwfo']) && intval($input['gwfo']) === 1
                ? true
                : false;
        $options['gwfo_loadmethod'] = trim($input['gwfo_loadmethod']);
        $options['gwfo_loadposition'] = trim($input['gwfo_loadposition']);
        $options['gwfo_googlefonts_auto'] =
            isset($input['gwfo_googlefonts_auto']) &&
            intval($input['gwfo_googlefonts_auto']) === 1
                ? true
                : false;
         ...
         * Google Fonts
         */
        $options['gwfo_googlefonts'] = isset($input['gwfo_googlefonts'])
            ? $this->CTRL->admin->newline_array($input['gwfo_googlefonts'])
            : array();
        ...
        if ($options['gwfo_config'] !== '') {
            ...
            if (
                ...
                $error = true;
                $this->CTRL->admin->set_notice(
                    'WebFontConfig variable is not valid. It should consist of <code>WebFontConfig = { ... };</code>.',
                    'ERROR'
                );
         ...
         */
        $options['gwfo_googlefonts_remove'] = isset(
         ...
         */
        $options['gwfo_googlefonts_ignore'] = isset(
            $input['gwfo_googlefonts_ignore']
        )
            ? $this->CTRL->admin->newline_array(
                $input['gwfo_googlefonts_ignore']
            )
        ...
        $this->CTRL->admin->save_settings(
            $options,
            'CSS optimization settings saved.'
        );
