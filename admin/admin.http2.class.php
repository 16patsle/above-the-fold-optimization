...
class ABTFR_Admin_HTTP2 {
    ...
    public function update_settings() {
         ...
         */
        $options['http2_push'] =
            isset($input['http2_push']) && intval($input['http2_push']) === 1
                ? true
                : false;
        $options['http2_push_config'] =
            isset($input['http2_push_config']) &&
            trim($input['http2_push_config']) !== ''
                ? $input['http2_push_config']
                : '';
        if ($options['http2_push_config']) {
            ...
            if (!is_array($options['http2_push_config'])) {
                ...
                $this->CTRL->admin->set_notice(
                    'The HTML/2 push config does not contain valid JSON.',
                    'ERROR'
                );
            }
        }
        ...
        $this->CTRL->admin->save_settings(
            $options,
            'HTTP/2 Optimization settings saved.'
        );
