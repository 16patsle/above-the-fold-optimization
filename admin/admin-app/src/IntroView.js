...
class IntroView extends Component {
    ...
    render() {
        ...
        const thinkurl = `${thinkhost}intl/${google_intlcode}?url=${encodeURIComponent(home_url)}`;

        const monitorurl = new URL(window.admin_url)
                            ...
                            <div className="postbox">
                                ...
                                <div className="inside testcontent">
                                    ...
                                    <p>
                                        ...
                                        <a className="button button-small" href={`https://developers.google.com/speed/pagespeed/insights/?url=${encodeURIComponent(home_url)}&hl=${lgcode}`} target="_blank">Google PageSpeed Test</a>
                                        <a className="button button-small" href={`https://search.google.com/search-console/mobile-friendly?url=${encodeURIComponent(home_url)}&hl=${lgcode}`} target="_blank">Google Mobile Test</a>
                                        <a className="button button-small" href={`https://performance.sucuri.net/domain/${encodeURIComponent(new URL(home_url).host.replace('www', ''))}`} target="_blank">Securi Server Response Time</a>
                                        <a className="button button-small" href={`http://www.webpagetest.org/?url=${encodeURIComponent(home_url)}`} target="_blank">WebPageTest.org</a>
                                        <a className="button button-small" href={`https://gtmetrix.com/?url=${encodeURIComponent(home_url)}`} target="_blank">GTmetrix</a>
                                        <a className="button button-small" href={`https://securityheaders.io/?q=${encodeURIComponent(home_url)}&hide=on&followRedirects=on`} target="_blank">SecurityHeaders.io</a>
                                        <a className="button button-small" href={`https://www.ssllabs.com/ssltest/analyze.html?d=${encodeURIComponent(home_url)}`} target="_blank">SSL test</a>
                                        <a className="button button-small" href={`http://www.intodns.com/${encodeURIComponent(new URL(home_url).host.replace('www', ''))}`} target="_blank">DNS test</a>
                                    </p>
                                    ...
                                    <p>Google <a href={`https://developers.google.com/speed/docs/insights/Server?hl=${lgcode}`} target="_blank">officially advises</a> a maximum server response time of 0,2 seconds (200ms). Test your global server response times <a href={`https://performance.sucuri.net/domain/${encodeURIComponent(new URL(home_url).host.replace('www', ''))}`} target="_blank">here</a>.</p>
                                    ...
                                    <p>Test your website at <a href={`https://developers.google.com/speed/pagespeed/insights/?url=${encodeURIComponent(home_url)}&hl=${lgcode}`} target="_blank">Google PageSpeed Insights</a> and start resolving the issues. Google offers detailed documentation for each issue and this plugin offers the tools to resolve most issues.</p>
